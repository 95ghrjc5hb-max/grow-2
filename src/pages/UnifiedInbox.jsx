...
export default function UnifiedInbox() {
  ...
  const getAuthHeaders = () => {
    // ১. সব জায়গা থেকে টোকেন খোঁজার চেষ্টা
    let token = localStorage.getItem("token") || localStorage.getItem("grow_access_token") || "";
    ...
    // ২. সমস্যা ধরার ফাঁদ (Alert)
    if (!token) {
      alert("🚨 Error: আপনার টোকেন একদম খালি! ব্রাউজার টোকেন সেভ করতে পারছে না।");
    }
  ...
  const fetchConversations = async () => {
    try {
      ...
      const convArray = Array.isArray(data.data) ? data.data : data || [];
 ...
 // Realtime subscription for incoming/outgoing messages
  ...
  const handleSend = async () => {
    ...
    // আগের মেসেজগুলোর সাথে নতুন মেসেজটা ডাটাবেসের ফরম্যাটে যোগ করা হচ্ছে
    const updatedMessages = [
      ...(selectedConv.messages || []),
      {
        sender: "agent",
        content: currentText, // 'text' এর বদলে ডাটাবেসের মতো 'content' হবে
        created_at: new Date().toISOString() // 'time' এর বদলে 'created_at' হবে
      }
    ];
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/message/send`, {
        ...
        body: JSON.stringify({
          ...
          customerPhone: selectedConv.customer_identifier,
      ...
      if (!res.ok) {
        console.error("Backend Auth/Send Error:", responseData);
            ...
            <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-none">
              {channelFilters.map((f) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${filter === f.key
              ...
              filtered.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 border-b border-white/5 transition-all ${isSelected ? "bg-teal-500/10 border-l-2 border-l-teal-500" : "hover:bg-white/[0.03]"
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${conv.ai_active
            ...
            <>
              <div className="p-4 border-b border-white/10 flex items-center justify-between bg-[hsl(220,18%,9%)]">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${selectedConv.ai_active
                      ? "bg-teal-500/10 text-teal-400 border border-teal-500/20"
                      : "bg-amber-500/10 text-amber-400 border border-amber-500/20"
                      }`}
                  >
                ...
                {!selectedConv.messages || selectedConv.messages.length === 0 ? (
                  ...
                  selectedConv.messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.sender === "customer" ? "justify-start" : "justify-end"}`}>
                      <div
                        className={`max-w-[70%] px-4 py-3 rounded-2xl text-sm ${msg.sender === "customer"
                      ...
                      >
                        ...
                        <p className="whitespace-pre-wrap">{msg.message || msg.text || msg.content || msg.message_body}</p>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03] border border-white/5">
                  ...
                  <span
                    className={`text-xs font-medium ${selectedConv.ai_active ? "text-teal-400" : "text-amber-400"
                      }`}
                  >
              ...
              <Button
                onClick={toggleAI}
                variant="outline"
                className={`w-full gap-2 ${selectedConv.ai_active
